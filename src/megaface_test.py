...
def get_parse():
    ...
    parser.add_argument('-n', '--model_name', default='r50')
    parser.add_argument('-p', '--model_path', default='backbone.pth')
    return parser.parse_args()

def build_model(model_name, fp16, model_path):
    if model_name == 'onnx':
...
def model_forward(model_name, model, inputs):
    if model_name == 'onnx':
        ...
        features = model(inputs_cuda).to('cpu').detach().numpy()
    return features
...
class MegaDataset(Dataset):
    def __init__(self, path_list, resize):
        self.images = []
        self.labels = []
        self.resize = resize
...
def get_mega_dataloader(root_dir, folder, batch_size, resize=(112, 112)):
    path_list = []
    ...
    dataset = MegaDataset(path_list, resize)
    ...
    for idx in range(len(face_dataloaders)):
        ...
        with tqdm(face_dict.items()) as t:
            for key, val in t:
                ...
                t.set_postfix(str=f'acc: {count[0]/count[1]:.4f}')

        if print_info: print(f'acc: {count[0]/count[1]:.4f}, {count[0]}/{count[1]} ------')
...
if __name__ == '__main__':
    ...
    model = build_model(args.model_name, args.fp16, args.model_path)
    print('model build done ------')

    face_folders = args.face_folders.split(',')
    face_dataloaders = [get_mega_dataloader(args.root_dir, folder, args.batch_size) for folder in face_folders]
    mega_dataloader = get_mega_dataloader(args.root_dir, args.mega_folder, args.batch_size)
